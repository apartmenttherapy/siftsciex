defmodule Siftsciex.Event.Payload.Promotion do
  @moduledoc """
  ...
  alias Siftsciex.Event.Payload.{CreditPoint, Discount}
  ...
  defstruct "$promotion_id": :empty,
            ...
            "$discount": :empty,
            "$credit_point": :empty
  @type t :: %__MODULE__{"$promotion_id": Payload.payload_string,
                         "$status": Payload.payload_string,
                         "$failure_reason": Payload.payload_string,
  ...
  """
  @spec new(data | [data]) :: __MODULE__.t | [__MODULE__.t]
  def new(promotion_data) when is_list(promotion_data) do
    promotion_data
    |> Enum.map(&(new(&1)))
  end
  ...
  defp normalize({:status, value}), do: {mark(:status), mark_string(value)}
